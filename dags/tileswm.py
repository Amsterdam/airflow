...
with DAG(
    ...
    trex_generate_vector_wm = KubernetesPodOperator(
        ...
        namespace="airflow",
        cmds=["/bin/bash"],
        arguments=["-c", "export DATABASE_PASSWORD=$(curl -s 'http://169.254.169.254/metadata/identity/oauth2/token?api-version=2018-02-01&resource=https%3A%2F%2Fossrdbms-aad.database.windows.net&client_id=60efcd71-1ca4-4650-ba7b-66f04c720d75' | jq -r .access_token); sleep 10m"],
        # arguments=["-c", "export DATABASE_PASSWORD=$(curl -s 'http://169.254.169.254/metadata/identity/oauth2/token?api-version=2018-02-01&resource=https%3A%2F%2Fossrdbms-aad.database.windows.net&client_id=60efcd71-1ca4-4650-ba7b-66f04c720d75' | jq -r .access_token); sleep 10m; t_rex generate --progress false --overwrite true --minzoom 10 --maxzoom 16 --extent 4.49712476945351,52.1630507756721,5.60867873764429,52.6147675426215 --config /var/config/topo_wm.toml"],
    ...
    #     namespace="airflow",
    #     cmds=["/bin/bash"],
    #     arguments=["-c", "azcopy login --identity --identity-client-id 60efcd71-1ca4-4650-ba7b-66f04c720d75; sleep 2m; azcopy copy '/var/cache/mvtcache/wm/*' https://piosupportstor.blob.core.windows.net/tiles/wm/ --recursive --content-encoding gzip --content-type application/vnd.mapbox-vector-tile"],
    ...
    #     namespace="airflow",
    #     arguments=["mapproxy-seed", "-c", "16", "-s", "/var/config/seed.yaml", "-f", "/var/config/mapproxy.yaml", "--seed=wm_kbk,wm_bgt"],
    ...
    #     image="dsoapi.azurecr.io/mapproxy",
    #     namespace="airflow",
    #     arguments=["mapproxy-seed", "-c", "16", "-s", "/var/config/seed.yaml", "-f", "/var/config/mapproxy.yaml", "--seed=wm_kbk_zw,wm_bgt_zw,wm_bgt_zw"],
    ...
    #     image="dsoapi.azurecr.io/mapproxy",
    #     namespace="airflow",
    #     arguments=["mapproxy-seed", "-c", "16", "-s", "/var/config/seed.yaml", "-f", "/var/config/mapproxy.yaml", "--seed=wm_kbk_light,wm_bgt_light"],
...
trex_generate_vector_wm
# trex_generate_vector_wm >> upload_vector_wm >> mapproxy_generate_tiles_wm >> mapproxy_generate_tiles_wm_zw >> mapproxy_generate_tiles_wm_light
